...
##### Useful Items
set bot_version "v2.0 build 071 (release)"
...
proc word_timer {} {
	putlog "DEBUG ##### Got to word_timer"
	global word_timer_id timer_minutes random_actions channel_name
	set timer_minutes [expr [rand 61] + 60]
	putlog "DEBUG ##### Got to word_timer A"
	#set word_timer_id [timer $timer_minutes word_timer]
	putlog "set word_timer_id [timer $timer_minutes word_timer]"
	putserv "PRIVMSG #callahans :\001ACTION [lindex $random_actions [rand [llength $random_actions]]]\001"
}
...
bind pubm - *Mike,~shoot* pub_shoot
bind pubm - *Mike,~channel~c90* pub_useless
bind pubm - *Mike,~wiki pub_wiki
...
proc pub_beer {nick mask hand chan arg} {
	...
	set beer [lindex $beer_list [rand [llength $beer_list]]]

	if {[lindex $arg 2] == "me"} {
		putserv "PRIVMSG $chan :\001ACTION serves $nick a $beer.\001"
		return
	}
	...
	if {[llength [lindex $arg 2]] > 4} {
		...
		if {$patron_local != $nick} {
			putserv "PRIVMSG $chan :\001ACTION serves $patron_local a $beer, courtesy of $nick.\001"
			return
		}
		if {$patron_local == $nick} {
			putserv "PRIVMSG $chan :\001ACTION serves $nick a $beer.\001"
...
proc pub_faq {nick mask hand chan arg} {
	putserv "PRIVMSG $chan :\001ACTION makes note that the channel FAQ can be found at http://unbaptisethedead.info/mike/viewtopic.php?t=56\001"
}
...
##### Trap for fortune
	...
	putlog "request == $request from pub_give for $nick"
	...
	if {$at_line_local == "absolutely nobody"} {
		...
		if {$valid_user_local == "1"} {
			...
			if {$xtra_status_local == "ON"} {
				set odds [rand 150]
				putlog "Got to valid user ($nick) with HELLO set to ON in $chan, with a value of $odds"
				set_gender $nick $chan
				if {$odds < 9} {
...
proc act:action_event {nick mask hand chan keyword arg} {
	...
	if {[lindex $arg 0] == "is" && [lindex $arg 1] == "away:"} {
		set odds [rand 300]
		if {$odds >= 0 && $odds <= 43} {
			putserv "PRIVMSG $chan :\001ACTION hangs a sign on $nick with 'Do Not Disturb until [expr [rand 27] + 1][lindex $month [rand [llength $month]]][expr [rand 15] + 2010]' on it.\001"
			return
		}
		if {$odds >= 44 && $odds <= 112} {
...
"pokes him like Michael Jackson at the Ranch."
...
proc pub_reset_pass {nick mask hand arg} {
	...
	if {![matchattr [nick2hand $nick] +Q]} {
		...
		putserv "PRIVMSG $nick :To change your temporary password, issue the following command:"
		putserv "PRIVMSG $nick :'/msg Callahan PASS <temppass> <newpass>'"	
		return
	}

	if [matchattr [nick2hand $nick] +Q] {
		#####If a bot op, reset the target's password, and /msg them the new one.
		set target_hand [lindex $arg 1]
		set target_nick [hand2nick $target_hand]
		set temp_pass [expr [rand 90000] + 10000]
		putlog "DEBUG PASS D $temp_pass"
		foreach bot_op [chanlist $chan +Q] {
			if {$bot_op != $nick} {
